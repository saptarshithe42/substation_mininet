...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    R2V_Mappings = {"10.0.0.1": "10.0.0.9", "10.0.0.2": "", "10.0.0.3": "", "10.0.0.4": "", "10.0.0.5": "", "10.0.0.6": "",
                    "10.0.0.7": "", "10.0.0.8": ""}
    V2R_Mappings = {"10.0.0.9": "10.0.0.1"}
    ...
    shuffling_excluded_ips = ["10.0.0.1"]
    ...
    def handleSwitchFeatures(self, ev):
        ...
        self.add_dns_intercept_flow_rule(datapath)
    ...
    def update_resources(self, ev):
        ...
        seed(time())
        exclusion_len = len(self.shuffling_excluded_ips)
        pseudo_ranum = randint(exclusion_len,
                               len(self.Resources) - 1)  # randint returns a random integer in the range of 0 and len(Resources)-1
        print(("Random Number:", pseudo_ranum))

        for keys in list(self.R2V_Mappings.keys()):
            if keys not in self.shuffling_excluded_ips:
                ...
                if pseudo_ranum == 0:
                    pseudo_ranum += exclusion_len

        self.V2R_Mappings = {v: k for k, v in list(self.R2V_Mappings.items())}
    ...
    def isDirectContact(self, datapath, ipAddr):
        ...
        '''
        if ipAddr in list(self.HostAttachments.keys()):
    ...
    # add dns interception flow rule
    def add_dns_intercept_flow_rule(self, datapath, priority=200):
        ...
        self.add_flow(datapath, priority, match_dns, actions_dns)
    ...
    def handlePacketInEvents(self, ev):
        ...
        dns_response = False
        dns_response_pkt = None

        if udp_pkt and udp_pkt.src_port == 53 and ip.tos == 0:
            dns_response = True
            ...
            # virtual_ip = "10.0.0.32"
            virtual_ip = self.R2V_Mappings[str(rr.rdata)]
            ...
            '''
            if self.isRealIPAddress(src) and src not in list(self.HostAttachments.keys()):
            ...
            if self.isVirtualIPAddress(dst):
                ...
                if self.isDirectContact(datapath=datapath.id, ipAddr=self.V2R_Mappings[dst]):
                    keys = list(self.V2R_Mappings.keys())
        ...
        dst = eth.dst
        src = eth.src
        '''Store the incoming packet source address, switch & the port combination to be used to learn the packet 
        switching'''
        ...
        '''
        if dns_response_pkt is not None:
            data = dns_response_pkt.data
            self.logger.info("dns response")
