...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def update_resources(self, ev):
        ...
        seed(time())
        pseudo_ranum = randint(
            0, len(self.Resources) - 1
        )  # randint returns a random integer in the range of 0 and len(Resources)-1
    ...
    def add_flow(
        self, datapath, priority, match, actions, buffer_id=None, hard_timeout=None
    ):
        """
    ...
    def handlePacketInEvents(self, ev):
        ...
        if arp_Obj:
            ...
            if self.isRealIPAddress(src):
                match = parser.OFPMatch(
                    eth_type=0x0806, in_port=in_port, arp_spa=src, arp_tpa=dst
                )
            ...
            if self.isVirtualIPAddress(dst):
                match = parser.OFPMatch(
                    eth_type=0x0806, in_port=in_port, arp_tpa=dst, arp_spa=src
                )
                if self.isDirectContact(
                    datapath=datapath.id, ipAddr=self.V2R_Mappings[dst]
                ):
            ...
            if self.isRealIPAddress(src):
                match = parser.OFPMatch(
                    eth_type=0x0800, in_port=in_port, ipv4_src=src, ipv4_dst=dst
                )
            ...
            if self.isVirtualIPAddress(dst):
                ...
                match = parser.OFPMatch(
                    eth_type=0x0800, in_port=in_port, ipv4_dst=dst, ipv4_src=src
                )
                if self.isDirectContact(
                    datapath=datapath.id, ipAddr=self.V2R_Mappings[dst]
                ):
                    ...
                    print(
                        "Changing DST Virtual IP " + dst + "---> Real DST IP " + ipDst
                    )
